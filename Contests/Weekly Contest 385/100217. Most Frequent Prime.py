...
class Solution:
    ...
    def mostFrequentPrime(self, mat: list[list[int]]) -> int:
        ...
        for i in range(rows):
            for j in range(cols):
                if mat[i-1][j-1]:
                   num = str(mat[i][j]) + str(mat[i-1][j-1])
                   if self.isprime(int(num)):
                    count[num] = count.get(num, 0) + 1

                if mat[i-1][j]:
                    num = str(mat[i][j]) + str(mat[i-1][j])
                    if self.isprime(int(num)):
                        count[num] = count.get(num, 0) + 1
...
print(Solution().mostFrequentPrime(mat = [[1,1],[9,9],[1,1]]))
