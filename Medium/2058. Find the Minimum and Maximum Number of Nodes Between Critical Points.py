    ...
    def nodesBetweenCriticalPoints(self, head):
        ...
        while curr and curr.next:
            if (prev.val > curr.val and curr.next.val > curr.val) or (prev.val < curr.val and curr.next.val < curr.val):
                # print(prev.val, curr.val, curr.next.val)
                ans.append(count)
            prev = prev.next
            curr = curr.next
            count+=1
        if len(ans) < 2: return [-1,-1]
        mindist = min([ans[i] - ans[i-1] for i in range(1, len(ans))])
        maxdist = ans[-1] - ans[0]
        return [mindist, maxdist]

listhead1 = Linkedlist().append([2,2,1,3])

head = Solution().nodesBetweenCriticalPoints(listhead1)
print(head)