...
class Solution(object):
    ...
    def longestZigZag(self, root):
        ...
        def dfs(node, curr_len, direction):
            if direction == "left":
                if node.right:
                    dfs(node.right, curr_len + 1, "right")
                else:
                    self.maxcount = max(curr_len, self.maxcount)
                if node.left:
                    dfs(node.left, 1, "left")
            else:
                if node.left:
                    dfs(node.left, curr_len + 1, "left")
                else:
                    self.maxcount = max(curr_len, self.maxcount)
                if node.right:
...
#Full referenced
root = BinaryTree().deserialize('[1,1,1,null,1,null,null,1,1,null,1]')
